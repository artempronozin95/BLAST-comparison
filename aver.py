...
data2 = pd.concat([df3,df4], ignore_index=True, axis=1)
dataall = pd.concat([data,data2], ignore_index=True, axis=1)
dataall.columns = ['Blast', 'Diamond', 'Usearch', 'BlastFast', 'k']
#K =100
...
#plt.plot(k[:bla], blast, color='green', linewidth = 3, label='Blast')
#plt.plot(k[:diam], dia, color='red', linewidth = 3,  label='Diamond')
#plt.plot(k[:usee], use, color='blue', linewidth = 3,  label='Usearch')
#plt.plot(k[:blaf], blastf, color='black', linewidth = 3, label='Blast-fast')
#plt.legend(loc = 'best')
#plt.xlabel('k')
#plt.ylabel('methods')
#plt.title(prot)
